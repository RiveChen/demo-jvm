...
// End-to-end: NEW + INVOKESPECIAL(<init>, Object.<init> no-op) + PUTFIELD +
// GETFIELD.
...
// INVOKEVIRTUAL: instance method reached via dynamic dispatch on the receiver's
// class.
TEST_F(InterpreterObjectTest, InvokeVirtualInstanceMethod) {
  EXPECT_EQ(executeStaticMethod<Jint>(kClassName, "callVirtual", 3, 4), 7);
}

// INVOKEVIRTUAL virtual dispatch: Animal a = new Dog(); a.sound() must select
// the override Dog.sound()=2 by the receiver's actual class, not
// Animal.sound()=1.
...
// (Before interning, LDC pushed a fresh dangling c_str() each time -> would be
// 0.)
